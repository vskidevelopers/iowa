...
function RoomCard({ title, imageUrl, price, roomType, small }) {
  return (
    <div className={`${small ? "px-1 md:px-20" : "px-0"}`}>
      <Link to="/" className="block overflow-clip max-w-xl">
        <img
          alt={title}
          src={imageUrl}
          className={`max-h-96 ${
        ...
        <p className="mt-2 max-w-sm text-gray-700">Price {price}</p>
